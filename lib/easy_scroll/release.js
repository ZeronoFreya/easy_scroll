...
// 只需把仲裁标记写入 runtimeData.countdown[axis].run，本函数读取内核状态即可。
import { clamp } from './utils.js'

/**
 * @param {object} runtimeData 内核运行状态（scroll / maxScroll / draging / back / countdown）
 * @param {'x'|'y'} [axis='y'] 需要归位的轴
 * @returns {boolean} true = 已执行归位
 */
export default function release(runtimeData, axis = 'y') {
    // 仍在拖动：不打断，位置继续由拖动方控制
    if (runtimeData.draging) return false
    // hint 倒计时进行中：保持过界位置，等待倒计时结束再归位
    if (runtimeData.countdown[axis].run) return false

    runtimeData.back = true
    runtimeData.scroll[axis] = clamp(runtimeData.scroll[axis], 0, runtimeData.maxScroll[axis])
    return true
}
