...
export default function useScrollbar(runtimeData, startLoop, signal, joyStickMode) {
    ...
    watch(
        () => runtimeData.scroll,
        (scroll) => {
            scrollRatio.x = safeDivide(scroll.left, runtimeData.contentSize.w)
            scrollRatio.y = safeDivide(scroll.top, runtimeData.contentSize.h)
            if (!joyStickMode && runtimeData.currCtrlType != 'scrollbar') {
    ...
    const updateScrollBar = () => {
        if (scrollRef.x.thumb) {
            ...
            thumbRect.x.width = Math.round(w * sizeRatio.x)
        ...
        if (scrollRef.y.thumb) {
            ...
            thumbRect.y.height = Math.round(h * sizeRatio.y)
    ...
    const resize = () => {
        scrollRatio.x = safeDivide(runtimeData.scroll.left, runtimeData.contentSize.w)
        scrollRatio.y = safeDivide(runtimeData.scroll.top, runtimeData.contentSize.h)

        sizeRatio.x = safeDivide(runtimeData.viewportSize.w, runtimeData.contentSize.w)
        sizeRatio.y = safeDivide(runtimeData.viewportSize.h, runtimeData.contentSize.h)
