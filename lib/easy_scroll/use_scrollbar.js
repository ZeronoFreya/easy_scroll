...
export default function useScrollbar(runtimeData, startLoop, joyStickMode) {
    ...
    if (!joyStickMode) {
        watch(
            ...
            (scroll) => {
                ...
                itemY.value = clamp(scroll.y <= 0 ? deltaY : -deltaY, 0, maxScrBarPos.y)
            },
            { deep: true },
        )
    }
    ...
    const updateMaxScrBarPos = () => {
        ...
        if (scrollRef.y.thumb) {
            itemH.value = scrollRef.y.track.offsetHeight * runtimeData.viewportSize.h  / runtimeData.contentSize.h
            maxScrBarPos.y = scrollRef.y.track.offsetHeight - itemH.value            
        }
    }
