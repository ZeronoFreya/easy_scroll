...
export default function useScrollbar(runtimeData, signal, joyStickMode, hint) {
    let itemStartY = 0
    ...
    const track_up = (e) => {
        ...
        // 释放后统一归位决策：倒计时中保持，否则快速回弹
        release(runtimeData, hint)
    }
    const maxPos = computed(() => {
        return joyStickMode
            ? maxScrBarPos.y * 0.5
            : runtimeData.maxOverscroll.y * runtimeData.sizeRatio.y
    })
    const track_move = (e) => {
        if (runtimeData.currCtrlType != 'scrollbar') return

        if (joyStickMode) {
            thumbRect.y.pos = clamp(e.clientY - itemStartY, -maxPos.value, maxPos.value)
        } else {
            thumbRect.y.pos = clamp(
                e.clientY - itemStartY,
                -maxPos.value,
                maxScrBarPos.y + maxPos.value,
            )            
        }
