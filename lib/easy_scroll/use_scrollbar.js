...
export default function useScrollbar(runtimeData, startLoop, signal, joyStickMode, hint) {
    ...
    const myLoop = () => {
        let velocity = thumbRect.y.pos * sensitivity.value * 0.5
        let ss = runtimeData.scroll.y - velocity
        runtimeData.scroll.y = clamp(ss, -runtimeData.maxScroll.y - 200, 200)
    ...
    const track_move = (e) => {
        ...
        if (joyStickMode) {
            ...
            if (runtimeData.scroll.y > 0) {
                // 顶部过界
                if (hint) {
                    hint.topPullRatio = runtimeData.scroll.y / 80
                }
            } else if (runtimeData.scroll.y < -runtimeData.maxScroll.y) {
                // 底部过界
                if (hint) {
                    hint.bottomPullRatio = Math.abs(runtimeData.scroll.y + runtimeData.maxScroll.y) / 80
                }
            } else {
                if (hint) {
                    hint.topPullRatio = 0
