...
export default function useScrollbar(runtimeData, signal, joyStickMode) {
    // 私有 rAF 句柄：不写入共享 runtimeData.rafId，
    // 避免与中键导航并行时互相 cancel 到对方的帧号。
    let rafId = null
    ...
    const myLoop = () => {
        ...
        if (loopContinue) {
            rafId = requestAnimationFrame(myLoop)
        }
    }
    ...
    const updateScrollBar = () => {
        if (scrollRef.x.thumb) {
            ...
            // 摇杆杆固定宽 100px(见 scrollbar.vue); 普通模式按内容比例。
            // 必须在此统一固定长度, 否则 maxScrBarPos 仍按比例小杆计算,
            // 拖动范围(±maxScrBarPos/2)会超出轨道, thumb 被推出轨外。
            thumbRect.x.width = joyStickMode ? 100 : Math.round(w * runtimeData.sizeRatio.x)
            maxScrBarPos.x = Math.ceil(w - thumbRect.x.width)
            maxScrRatio.x = safeDivide(runtimeData.maxScroll.x, maxScrBarPos.x)

            if (joyStickMode) {
                // 摇杆杆固定宽 100px, 以轨道中心为原点居中(margin 修正半宽)
                scrollRef.x.thumb.style.marginLeft = '-50px'
            }
        }
        if (scrollRef.y.thumb) {
            const h = scrollRef.y.track.offsetHeight
            thumbRect.y.height = joyStickMode ? 100 : Math.round(h * runtimeData.sizeRatio.y)
