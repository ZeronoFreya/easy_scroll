...
export default function useScrollbar(runtimeData, startLoop, signal, joyStickMode, hint) {
    ...
    const maxPos = computed(() => {
        return joyStickMode ? maxScrBarPos.y * 0.5 : overSize.value * runtimeData.sizeRatio.y
    })
    const track_move = (e) => {
        ...
        if (joyStickMode) {
            ...
            thumbRect.y.pos = clamp(e.clientY - itemStartY, -maxPos.value, maxScrBarPos.y + maxPos.value)
        }
