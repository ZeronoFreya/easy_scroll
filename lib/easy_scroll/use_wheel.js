...
export default function useWheel(runtimeData, startLoop, hint) {
    const cfg = reactive({
        // 灵敏度
        sensitivity: 0.05,
    })
    ...
    const onWheel = (e) => {
        runtimeData.scroll.y = clamp(
            runtimeData.scroll.y + e.wheelDeltaY,
            -runtimeData.maxScroll.y - overSize.value,
            overSize.value,
        )        
        // console.log(runtimeData.scroll.y, runtimeData.maxScroll.y);
        
        if (hint) {
            hint.resetCountdown(e.wheelDeltaX, e.wheelDeltaY)
        }
    }
    const onWheel2 = (e) => {
