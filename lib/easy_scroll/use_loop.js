...
export default function useLoop(runtimeData, scrollCtrl) {
    ...
    const physicsLoop = () => {
        ...
        if (runtimeData.isDragging) {
            activeInputY = scrollCtrl.stick.itemY.value
            isControlActive = true
            runtimeData.inputSource = 'stick'
            runtimeData.isWheelFreezing = false
            scrollCtrl.wheel.cancelTimer()

            runtimeData.velocity = -activeInputY * scrollCtrl.stick.cfg.sensitivity
        } else if (runtimeData.isMiddleMouseActive) {
            activeInputY = runtimeData.virtualItemY
            isControlActive = true
            ...
            runtimeData.velocity = -activeInputY * scrollCtrl.midnav.cfg.sensitivity
        } else {
