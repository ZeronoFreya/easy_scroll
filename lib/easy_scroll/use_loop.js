...
export default function useLoop(runtimeData, scrollCtrl) {
    ...
    const physicsLoop = () => {
        ...
        if (runtimeData.currCtrlType == 'scrollbar') {
            activeInputY = scrollCtrl.scroll.thumbRect.y.top
            ...
            if (scrollCtrl.scroll.joyMode) {
                ...
                let targetScrollY = safeDivide(activeInputY * h2, h)
                runtimeData.velocity = 0
                // 更新速度 (为了松手后的惯性)
                // runtimeData.velocity = -activeInputY * scrollCtrl.scroll.sensitivity
                runtimeData.scroll.top = -targetScrollY
                // console.log(targetScrollY , runtimeData.scroll.top);
        ...
        if (runtimeData.scroll.top > 0) {
            displacement = runtimeData.scroll.top
            runtimeData.isOverscrolling = true
            scrollCtrl.hint.topPullRatio = runtimeData.scroll.top / config.pullThreshold
        } else if (runtimeData.scroll.top < -runtimeData.maxScroll.top) {
            displacement = runtimeData.scroll.top + runtimeData.maxScroll.top
            runtimeData.isOverscrolling = true
            scrollCtrl.hint.bottomPullRatio = Math.abs(displacement) / config.pullThreshold            
        } else {
            if (scrollCtrl.hint.topPullRatio > 0) scrollCtrl.hint.topPullRatio = 0
            if (scrollCtrl.hint.bottomPullRatio > 0) scrollCtrl.hint.bottomPullRatio = 0
        }
        ...
        if (displacement !== 0) {
            ...
            if (
                ...
                runtimeData.scroll.top = displacement > 0 ? 0 : -runtimeData.maxScroll.top
                runtimeData.velocity = 0
                runtimeData.isWheelFreezing = false
            }
        }

        let nextScrollY = runtimeData.scroll.top + runtimeData.velocity
        ...
        if (!isControlActive) {
            ...
            if (!runtimeData.isWheelFreezing) {
                if (runtimeData.scroll.top > 0 && nextScrollY <= 0) {
                    nextScrollY = 0
                    runtimeData.velocity = -runtimeData.velocity * restitution
                } else if (
                    runtimeData.scroll.top < -runtimeData.maxScroll.top &&
                    nextScrollY >= -runtimeData.maxScroll.top
                ) {
                    nextScrollY = -runtimeData.maxScroll.top
                    runtimeData.velocity = -runtimeData.velocity * restitution
                }
            ...
            if (nextScrollY < -runtimeData.maxScroll.top - limit)
                nextScrollY = -runtimeData.maxScroll.top - limit
            if (nextScrollY > limit) nextScrollY = limit
        }

        runtimeData.scroll.top = nextScrollY
        ...
        if (!runtimeData.isOverscrolling && !isControlActive && !runtimeData.isWheelFreezing) {
            if (Math.abs(runtimeData.velocity) < 0.1) {
                // 靠近顶部
                if (Math.abs(runtimeData.scroll.top) < 0.2) {
                    runtimeData.scroll.top = 0
                    runtimeData.velocity = 0
                }
                // 靠近底部
                else if (Math.abs(runtimeData.scroll.top + runtimeData.maxScroll.top) < 0.2) {
                    runtimeData.scroll.top = -runtimeData.maxScroll.top
                    runtimeData.velocity = 0
                }
            }
